...
interface InspectorPedagogicalProfileProps {
  ...
  onSetTeacherSubTab: (
    tab: 'annual_plan' | 'schedule' | 'lesson_plans' | 'students' | 'visits'
  ) => void;
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-800/80 p-3.5 rounded-2xl text-xs border border-slate-700/60">
          ...
          <div>
            ...
            <span className="font-black text-purple-300 text-sm">
              {weeklyHoursCount} ساعة / أسبوعياً
            </span>
          </div>
          <div>
          ...
          <div>
            ...
            <span className="font-black text-slate-300 text-sm">غير متاح حالياً</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 space-y-4">
        ...
        {teacherSubTab === 'annual_plan' &&
          (() => {
            ...
            const selectedLevelNotebook = teacherNotebook.filter(
              (entry) => entry.status === 'منجزة'
            );

            const matchesDomain1 = (item: any) => {
              const text =
                `${item.fieldName || ''} ${item.competencyTitle || ''} ${item.segmentTitle || ''} ${item.sessionTitle || ''} ${item.note || ''} ${item.domain || ''}`.toLowerCase();
              return (
              ...
              <div className="space-y-4 animate-in fade-in duration-150">
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    <div className="text-right">
                      <span className="text-slate-400 text-[10px] block">
                        إجمالي الحصص البيداغوجية
                      </span>
                      <span className="font-extrabold text-amber-400">
                        30 حصة (10 حصص لكل ميدان)
                      </span>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-2">
                    <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                      <span className="text-xs font-black text-slate-900">
                        الفصل 1: الميدان 1 (10 حصص)
                      </span>
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${locCompleted === 10 ? 'bg-emerald-100 text-emerald-800' : locCompleted > 0 ? 'bg-blue-100 text-blue-800' : 'bg-slate-100 text-slate-600'}`}
                      >
                        {Math.round((locCompleted / 10) * 100)}%
                      </span>
                    </div>
                    <p className="text-[11px] text-slate-600 font-medium">
                      الوضعيات والتنقلات: التحكم في وضعيات الجسم والتوازن في الفضاء.
                    </p>
                    <div className="w-full bg-slate-100 rounded-full h-2">
                      <div
                        className="bg-emerald-600 h-2 rounded-full transition-all duration-300"
                        style={{ width: `${(locCompleted / 10) * 100}%` }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-2">
                    <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                      <span className="text-xs font-black text-slate-900">
                        الفصل 2: الميدان 2 (10 حصص)
                      </span>
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${fundCompleted === 10 ? 'bg-emerald-100 text-emerald-800' : fundCompleted > 0 ? 'bg-blue-100 text-blue-800' : 'bg-slate-100 text-slate-600'}`}
                      >
                        {Math.round((fundCompleted / 10) * 100)}%
                      </span>
                    </div>
                    <p className="text-[11px] text-slate-600 font-medium">
                      الحركات القاعدية: اكتساب وتوظيف المشي والجري والقفز والرمي.
                    </p>
                    <div className="w-full bg-slate-100 rounded-full h-2">
                      <div
                        className="bg-blue-600 h-2 rounded-full transition-all duration-300"
                        style={{ width: `${(fundCompleted / 10) * 100}%` }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-2">
                    <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                      <span className="text-xs font-black text-slate-900">
                        الفصل 3: الميدان 3 (10 حصص)
                      </span>
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${structCompleted === 10 ? 'bg-emerald-100 text-emerald-800' : structCompleted > 0 ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-600'}`}
                      >
                        {Math.round((structCompleted / 10) * 100)}%
                      </span>
                    </div>
                    <p className="text-[11px] text-slate-600 font-medium">
                      الهيكلة والبناء: الأنشطة الجماعية، احترام القواعد والتعاون.
                    </p>
                    <div className="w-full bg-slate-100 rounded-full h-2">
                      <div
                        className="bg-amber-500 h-2 rounded-full transition-all duration-300"
                        style={{ width: `${(structCompleted / 10) * 100}%` }}
                      ></div>
                    </div>
                  </div>
                </div>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      <tr className="hover:bg-slate-50">
                        <td className="p-3 font-bold text-slate-900">
                          {levelData.fields.f_locomotion?.fieldName ||
                        ...
                        <td className="p-3 text-center">
                          <span
                            className={`font-bold px-2.5 py-1 rounded-xl text-[10px] ${locCompleted === 10 ? 'bg-emerald-100 text-emerald-800' : locCompleted > 0 ? 'bg-blue-100 text-blue-800' : 'bg-slate-100 text-slate-600'}`}
                          >
                            {locCompleted}/10 حصص
                          </span>
                        </td>
                      ...
                      <tr className="hover:bg-slate-50">
                        <td className="p-3 font-bold text-slate-900">
                          {levelData.fields.f_fundamentals?.fieldName ||
                            'الميدان الثاني: الحركات القاعدية'}
                        </td>
                        <td className="p-3 text-slate-700 max-w-xs">
                        ...
                        <td className="p-3 text-center">
                          <span
                            className={`font-bold px-2.5 py-1 rounded-xl text-[10px] ${fundCompleted === 10 ? 'bg-emerald-100 text-emerald-800' : fundCompleted > 0 ? 'bg-blue-100 text-blue-800' : 'bg-slate-100 text-slate-600'}`}
                          >
                      ...
                      <tr className="hover:bg-slate-50">
                        <td className="p-3 font-bold text-slate-900">
                          {levelData.fields.f_structuring?.fieldName ||
                            'الميدان الثالث: الهيكلة والبناء'}
                        </td>
                        <td className="p-3 text-slate-700 max-w-xs">
                        ...
                        <td className="p-3 text-center">
                          <span
                            className={`font-bold px-2.5 py-1 rounded-xl text-[10px] ${structCompleted === 10 ? 'bg-emerald-100 text-emerald-800' : structCompleted > 0 ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-600'}`}
                          >
          ...
          <div className="space-y-4 animate-in fade-in duration-150">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
              <h4 className="text-xs font-extrabold text-slate-900 flex items-center gap-1.5">
                ...
                <span>
                  المذكرات البيداغوجية المحضرة من قبل الأستاذ ({teacherLessonPlans.length} مذكرات)
                </span>
              </h4>
            ...
            <div className="bg-gradient-to-r from-blue-900 to-indigo-900 text-white p-5 rounded-2xl shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-blue-200 mt-1">
                  موزعين على {teacherClasses.length} أقسام مسندة بـ{' '}
                  {selectedTeacher.schoolName || 'المدرسة الابتدائية'}
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {teacherClasses.map((cls) => {
                ...
                return (
                  <div
                    key={cls.id}
                    className="p-4 bg-white rounded-2xl border border-slate-200 space-y-2"
                  >
                    ...
                    <div className="text-[11px] text-slate-500 space-y-1">
                      <div>
                        الذكور: <strong>{Math.round(count * 0.52)}</strong> | الإناث:{' '}
                        <strong>{Math.round(count * 0.48)}</strong>
                      </div>
                      <div>
                        النوادي: نادي أ ({Math.round(count / 2)}) • نادي ب (
          ...
          <div className="space-y-4 animate-in fade-in duration-150">
            ...
            {visits.filter((v) => v.teacherId === selectedTeacher.id).length === 0 ? (
              <p className="text-xs text-slate-400 italic bg-slate-50 p-4 rounded-2xl text-center">
                لا توجد زيارات تفتيشية مؤرشفة لهذا الأستاذ حالياً. يمكنك إضافة زيارة جديدة عبر الزر
                أعلاه.
              </p>
            ) : (
              visits
                ...
                .map((v) => (
                  ...
                  >
                    ...
                    <p className="text-xs font-bold text-slate-800">
                      الحصة الملاحظة: {v.lessonObservedTitle}
                    </p>
              ...
              {notes.filter((n) => n.teacherId === selectedTeacher.id).length === 0 ? (
                ...
                notes
                  ...
                  .map((n) => (
                    <div
                      key={n.id}
                      className="p-3.5 rounded-2xl bg-slate-900 text-white space-y-1.5 text-xs"
                    >
                      <div className="flex items-center justify-between">
