...
export const InspectorWorkspacePage: React.FC<Props> = (props) => {
  ...
  if (module === 'inspector_teachers' && props.teacherId) {
    ...
    return <div className="space-y-5 dir-rtl">
      ...
      <div className="flex flex-wrap gap-2"><button onClick={() => props.onNavigateWithTeacher?.('inspector_visits', props.teacherId!)} className="rounded-xl bg-emerald-600 px-4 py-2 text-xs font-bold text-white">إضافة زيارة</button><button onClick={() => props.onNavigateWithTeacher?.('inspector_guidance', props.teacherId!)} className="rounded-xl bg-slate-800 px-4 py-2 text-xs font-bold text-white">إضافة توجيه</button><button onClick={() => props.onNavigateWithTeacher?.('inspector_communication', props.teacherId!)} className="rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white">مراسلة الأستاذ</button></div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-black">الزيارات والتوجيهات</h2>{detail.visits?.length ? <div className="mt-3 space-y-2">{detail.visits.map((visit: any) => <div key={visit.id || `${visit.visitDate}-${visit.lessonObservedTitle}`} className="rounded-xl bg-slate-50 p-3 text-sm"><div className="flex items-center justify-between gap-2"><span className="font-bold">{visit.visitDate || visit.createdAt || 'غير محدد'} — {visit.visitType || 'زيارة'}</span><span className="text-xs text-slate-500">{visit.lessonObservedTitle || 'معاينة بيداغوجية'}</span></div>{visit.positivePoints?.length ? <p className="mt-1 text-xs text-slate-600">{visit.positivePoints.join(' • ')}</p> : null}</div>)}</div> : <p className="mt-3 text-sm text-slate-500">لا توجد زيارات مسجلة لهذا الأستاذ.</p>}{detail.guidance?.length ? <p className="mt-2 text-sm">آخر توجيه: {detail.guidance[0]?.title || 'توجيه بيداغوجي'}</p> : <p className="mt-2 text-sm text-slate-500">لا توجد توجيهات أو ملاحظات مسجلة.</p>}</section>
    </div>;
  }
  if (props.teacherId && !selectedTeacher) return <div className="space-y-4 rounded-2xl border border-rose-200 bg-rose-50 p-6 text-sm font-bold text-rose-800">تعذر فتح توجيه الأستاذ أو لم يعد ضمن الأساتذة المسندين إليك.<button onClick={() => props.onNavigate('inspector_teachers')} className="block mt-3 text-emerald-700 underline">العودة إلى أساتذة المقاطعة</button></div>;
  ...
  if (module === 'inspector_visits') return <section className="space-y-5"><h1 className="text-lg font-black flex items-center gap-2"><FileSpreadsheet className="text-emerald-600" />تقارير وتوجيهات المعاينات</h1><InspectorReportsView visits={visits} teachers={teachers} inspector={inspector} teacherId={props.teacherId} onAddVisit={props.onAddVisit} onVisitSaved={async (teacherId) => { await props.onRefreshVisits(); await props.onRefreshTeachers(); window.dispatchEvent(new CustomEvent('inspector-visit-saved', { detail: { teacherId } })); }} onClearTeacherContext={() => props.onNavigate('inspector_visits')} /></section>;
