    ...
    <div className="space-y-6 animate-in fade-in duration-200 dir-rtl">
      <section className="rounded-3xl bg-gradient-to-br from-emerald-950 via-slate-900 to-teal-950 p-6 text-white shadow-md border border-emerald-800/40">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black">الرئيسية</h1>
            <p className="text-xs text-emerald-100/80">
              ملخص مساحة الإشراف البيداغوجي — {inspector.firstName} {inspector.lastName}
            </p>
          </div>
        </div>
      </section>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {cards.map(({ tab, label, value, icon: Icon, color }) => (
          <button
            key={label}
            onClick={() => onNavigateTab?.(tab)}
            className="text-right rounded-2xl border border-slate-200 bg-white p-5 shadow-xs hover:shadow-md transition-shadow"
          >
