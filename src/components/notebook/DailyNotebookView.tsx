    ...
    >
      <header className="workspace-header workspace-notebook-controls space-y-5 rounded-3xl border border-slate-200/80 bg-white p-5 shadow-xs sm:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div>
            ...
            <h1 className="mt-3 flex items-center gap-2 text-2xl font-bold text-slate-950">
          ...
          <div className="grid gap-3 text-xs font-bold text-slate-700 sm:grid-cols-2 xl:min-w-[42rem] xl:grid-cols-3">
            ...
            {classFilter === 'all' && (
              <span className="text-xs font-normal text-slate-500 sm:col-span-2 xl:col-span-3">
        ...
        <div className="grid gap-3 border-y border-slate-100 py-4 sm:grid-cols-2 lg:grid-cols-4">
          {currentUser.schoolName && (
            <div className="rounded-2xl bg-slate-50 px-4 py-3">
              <span className="block text-xs font-bold text-slate-500">المؤسسة</span>
          ...
          <div className="rounded-2xl bg-slate-50 px-4 py-3">
            <span className="block text-xs font-bold text-slate-500">السنة الدراسية</span>
          ...
          {teacherDisplayName(currentUser) && (
            <div className="rounded-2xl bg-slate-50 px-4 py-3">
              <span className="block text-xs font-bold text-slate-500">الأستاذ</span>
              <strong className="mt-1 block text-sm text-slate-900">
                {teacherDisplayName(currentUser)}
              </strong>
            </div>
          ...
          {commonDomain && (
            <div className="rounded-2xl bg-blue-50 px-4 py-3">
              <span className="block text-xs font-bold text-blue-700">الميدان</span>
          ...
          <div className="flex min-w-0 flex-1 gap-1 overflow-x-auto sm:justify-center">
            {weekDates.map((date, index) => {
              ...
              return (
                ...
                >
                  <span className="block font-bold">{WEEKDAY_LABELS[index]}</span>
                  <span className="mt-0.5 block text-xs">{dayNumber(date)}</span>
                  <span
                    className={`mt-1 block text-xs font-medium ${isSelected ? 'text-blue-100' : 'text-slate-400'}`}
                  >
      ...
      {safeTeacherClasses.length === 0 && (
        <div className="workspace-empty-state rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center">
          ...
          <h2 className="font-bold text-slate-900">لا توجد أقسام مسندة إليك بعد.</h2>
            ...
            >
              ...
              <div className="grid gap-px bg-slate-200 lg:grid-cols-[1.05fr_1.2fr_1.35fr_1.1fr_1.55fr]">
                <section className="bg-white p-4">
                  <h2 className="text-sm font-bold text-blue-800">التاريخ / القسم / التوقيت</h2>
                  <div className="mt-3 space-y-2 text-sm text-slate-700">
                    <div>
                      <span className="block text-xs font-bold text-slate-500">التاريخ</span>
                    ...
                    {sessionClass && (
                      <>
                        <div>
                          <span className="block text-xs font-bold text-slate-500">المستوى</span>
                          <strong className="mt-0.5 block text-slate-950">
                            {levelLabel(sessionClass.levelId)}
                          </strong>
                        </div>
                        <div>
                          <span className="block text-xs font-bold text-slate-500">الفوج</span>
                          <strong className="mt-0.5 block text-slate-950">
                            {sessionClass.name}
                          </strong>
                        </div>
                    ...
                    {sessionDto.startTime && (
                      <div>
                        <span className="block text-xs font-bold text-slate-500">التوقيت</span>
                        <strong className="mt-0.5 block text-slate-950" dir="ltr">
                          {sessionDto.startTime}{' '}
                ...
                <section className="bg-white p-4">
                  <h2 className="text-sm font-bold text-blue-800">التعلمات</h2>
                ...
                <section className="bg-white p-4">
                  <h2 className="text-sm font-bold text-blue-800">محتوى التعلم</h2>
                  {content.length > 0 && (
                ...
                <section className="bg-white p-4">
                  <h2 className="text-sm font-bold text-blue-800">المذكرة</h2>
                ...
                <section className="bg-white p-4">
                  <h2 className="text-sm font-bold text-blue-800">الملاحظات</h2>
                  <textarea
              ...
              {isPreviewExpanded && memoPreview && (
                <div className="space-y-3 rounded-2xl border border-blue-100 bg-blue-50/50 p-4 text-xs text-slate-700">
                  <div className="flex flex-wrap gap-x-5 gap-y-2">
                    {memoPreview.equipment.length > 0 && (
                      <p>
                        <span className="font-bold text-blue-950">الوسائل:</span>{' '}
                        {memoPreview.equipment.join('، ')}
                      </p>
                    )}
                    {memoPreview.contentSummary && (
                      <p>
                        <span className="font-bold text-blue-950">محتوى مختصر:</span>{' '}
                        {memoPreview.contentSummary}
                      </p>
                    )}
                  </div>
                  {memoPreview.situations.length > 0 && (
                    <div>
                      <p className="font-bold text-blue-950">المواقف التربوية:</p>
