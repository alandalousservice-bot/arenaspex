...
describe('Inspector teacher follow-up workspace', () => {
  it('uses a server-authorized detail read model', () => {
    ...
    expect(source).toContain("'/inspector/teachers/:teacherId/follow-up'");
  ...
  it('supports a dedicated teacher detail URL and read-only follow-up sections', () => {
    ...
    expect(read('src/components/dashboard/inspector/InspectorReportsView.tsx')).toContain(
      'teacherId?: string'
    );
  });
  ...
  it('uses InspectorNote for authorized teacher-specific guidance and preserves district mode', () => {
    ...
    expect(read('src/components/dashboard/InspectorWorkspacePage.tsx')).toContain(
      'تعذر فتح توجيه الأستاذ'
    );
  });
  ...
  it('keeps roster actions on accepted teachers and avoids per-card fetches', () => {
    ...
    expect(read('src/components/dashboard/inspector/InspectorTeacherList.tsx')).toContain(
      'فتح ملف المتابعة'
    );
