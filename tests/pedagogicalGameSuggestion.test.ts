...
import {
  canEditSuggestedGame,
  canReviewSuggestedGame,
  hasProbableGameDuplicate,
  validateGameSuggestion,
} from '../src/services/pedagogicalGameSuggestion.service';

const context = {
  grade: 3,
  fieldId: 'f_fundamentals',
  fieldName: 'الحركات القاعدية',
  objectiveId: 'curriculum_objective_1',
  objectiveText: 'ينجز القفز والوثب مع التحكم في الارتقاء والهبوط.',
};

describe('pedagogical game suggestion workflow', () => {
  it('preserves grade, field and objective and starts as a draft', () => {
    const item = validateGameSuggestion(
      { title: 'لعبة القفز', description: 'تدريب القفز', rules: 'يقفز المتعلمون بالتناوب.' },
      context
    );
  ...
  it('rejects empty generated shells and detects probable duplicates', () => {
    ...
    expect(hasProbableGameDuplicate({ title: 'لعبة القفز' }, existing, 3, context.fieldId)).toBe(
      true
    );
  });
  ...
  it('allows only owner draft/rejected editing and reviewer approval', () => {
    ...
    expect(canReviewSuggestedGame('teacher', { ...item, approvalStatus: 'PENDING_APPROVAL' })).toBe(
      false
    );
    expect(
      canReviewSuggestedGame('inspector', { ...item, approvalStatus: 'PENDING_APPROVAL' })
    ).toBe(true);
    expect(canReviewSuggestedGame('admin', { ...item, approvalStatus: 'PENDING_APPROVAL' })).toBe(
      true
    );
  });
  ...
  it('keeps technical generation metadata out of visible Knowledge Bank wording', () => {
    ...
    expect(source).toContain('اقتراح موقف تربوي');
