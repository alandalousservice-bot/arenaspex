...
describe('Grade 4/5 learning sequence redistribution', () => {
  it.each(['lvl_p4', 'lvl_p5'] as const)(
    'generates complete 7/6/5 domain paths for %s',
    (levelId) => {
      ...
      for (const domainId of ['f_locomotion', 'f_fundamentals', 'f_structuring']) {
        ...
        expect(learning.length).toBe(
          levelId === 'lvl_p4'
            ? { f_locomotion: 14, f_fundamentals: 12, f_structuring: 10 }[domainId]
            : { f_locomotion: 7, f_fundamentals: 6, f_structuring: 5 }[domainId]
        );
