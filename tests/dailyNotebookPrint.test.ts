...
describe('Daily Notebook dedicated print document', () => {
  ...
  it('exposes a dedicated print-only document and scoped landscape CSS', () => {
    ...
    expect(printDocument).toContain('daily-print-page');
    ...
    const dailyPageRule = css.match(/@page daily-notebook\s*\{([\s\S]*?)\}/)?.[1] ?? '';
    ...
    expect(css).toContain('grid-template-columns: 8% 12% 27% 30% 8% 15%;');
    expect(css).toContain('font-size: 9.5pt;');
    expect(css).toContain('width: 8%;');
    expect(css).toContain('width: 12%;');
    expect(css).toContain('width: 27%;');
    expect(css).toContain('width: 30%;');
    expect(css).toContain('width: 8%;');
    expect(css).toContain('width: 15%;');
